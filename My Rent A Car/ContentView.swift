...
struct ContentView: View {
    ...
    @State private var isReportEmpty: Bool = false
            ...
            NavigationView {
                ScrollView {
                    VStack {
                        ...
                        Button(action: {
                            if customerViewModel.customer.name.isEmpty || customerViewModel.customer.address.isEmpty || customerViewModel.customer.idOrPassport.isEmpty {
                                // Show alert for empty report
                                isReportEmpty = true
                            } else {
                                let report = Report(customer: customerViewModel.customer, car: carViewModel.car)
                                reports.append(report)
                                // Set the state variable to true when the report is saved
                                isReportSavedSuccessfully = true
                        ...
                        .alert(isPresented: .constant(isReportEmpty || isReportSavedSuccessfully)) {
                            if isReportEmpty {
                                return Alert(title: Text("Error"), message: Text("Please fill in all the required fields to generate the report."), dismissButton: .default(Text("OK")))
                            } else {
                                return Alert(title: Text("Success"), message: Text("Report saved successfully."), dismissButton: .default(Text("OK")))
                            }
                        }
                        .padding()
